# -*- coding: utf-8 -*-
"""
Resample data to 8 hour averages.
...
n_tst_weeks = 54-n_tr_weeks-n_val_weeks
n_tr = n_tr_weeks*7*24
n_val = n_val_weeks*7*24
n_tst = n_tst_weeks*7*24
...
for c, code in enumerate(codes):
    ...
    resampled_series = series.resample('8h').mean()
    # Weekly ordering
    for idx in range(n_tr_weeks):
        tr_clustering_matrix[tr_cont, :] = resampled_series[idx*periods:idx*periods+periods]
        tr_code_index.append(code)
        tr_cont += 1
    for idx in range(n_tr_weeks, n_tr_weeks+n_val_weeks):
        val_clustering_matrix[val_cont, :] = resampled_series[idx*periods:idx*periods+periods]
...
with open(resources.clustering_matrices_8h, 'w') as f:
